...
(function(window, angular) {
    ...
    TestReport.prototype.getViewBreakdown = function() {
        if (this.viewBreakdown) {
            return this.viewBreakdown;
        }

        this.viewBreakdown = {};

        this.cases.forEach(function(tc) {
            this.viewBreakdown[tc.job.view] = this.viewBreakdown[tc.job.view] || [];
            this.viewBreakdown[tc.job.view].push(tc);
        }, this);

        return Object.keys(this.viewBreakdown).map(function(viewName) {
            var tests = this.viewBreakdown[viewName];
            return {
                view: viewName,
                tests: tests.length,
                passRate: tests.filter(function(t) { return t.passing }).length,
                failRate: tests.filter(function(t) { return t.failing }).length,
                unstableRate: tests.filter(function(t) { return t.unstable }).length,
                skipRate: tests.filter(function(t) { return t.skipped }).length,
            };
    ...
    jenkins.provider('jenkins', function JenkinsProvider() {
        this.$get = ['$http', '$base64', 'configuration', '$rootScope',
